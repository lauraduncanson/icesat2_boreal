...
def write_local_data_and_catalog_s3__(catalog, bands_dict, save_path, local, s3_path="s3://"):
...
def write_local_data_and_catalog_s3(catalog, bands_dict, comp_type, save_path, local, s3_path="s3://maap-ops-dataset/maap-users/alexdevseed/landsat8/sample2/"):
    ...
    with open(catalog) as f:
        ...
        for feature in features:
            print(feature['id']) #  Check if feature can tell us if LT05 LE07 or LC08
            product = feature['id'].split('_')[0]
            bands = bands_dict[product]
            try:
                for band in bands:
                    if comp_type == 'LC2SR':
...
MS_BANDS_DICT = dict({
                        ...
                        'LC08': ['blue', 'green', 'red', 'nir08', 'swir16', 'swir22','cloud_qa'],
                      })
...
def get_HLS_data(in_tile_fn, in_tile_layer, in_tile_id_col, in_tile_num, out_dir, sat_api, 
                 ...
                 hls_product='L30', hls_product_version='2.0', min_n_filt_results=0, bands_dict=MS_BANDS_DICT):
    ...
    master_json = write_local_data_and_catalog_s3__(master_json, bands_dict, save_path, local, s3_path="s3://")
    
    return master_json

def get_LC2SR_data(in_tile_fn, in_tile_layer, in_tile_id_col, in_tile_num, out_dir, sat_api, 
                 start_year, end_year, start_month_day, end_month_day, max_cloud, local=False, 
                 #hls_product='L30', hls_product_version='2.0', 
                   min_n_filt_results=0, bands_dict=MS_BANDS_DICT):
      
    geojson_path_albers = in_tile_fn
    layer = in_tile_layer
    tile_n = int(in_tile_num)
    
    print('\nGetting Landsat Collection 2 (L2) Surface Reflectance data...')
    ...
    for bbox in bbox_list:
        # Geojson of total scenes - Change to list of scenes
        print(f'bbox: {bbox}')
        response_by_year = [query_stac(year, bbox, max_cloud, api, start_month_day, end_month_day, MS_product='landsat-c2l2-sr', 
                                       MS_product_version=None, MIN_N_FILT_RESULTS=min_n_filt_results) for year in years]
    ...
    #
    # Write local JSON that catalogs the LC2SR data retrieved from query
    #
    master_json = os.path.join(save_path, f'master_{tile_n}_{np.min(years)}-{start_month_day}_{np.max(years)}-{end_month_day}_LC2SR.json')
    with open(master_json, 'w') as outfile:
            json.dump(merge_catalogs, outfile)

    master_json = write_local_data_and_catalog_s3(master_json, bands_dict, 'LC2SR', save_path, local, s3_path="s3://usgs-landsat/")
