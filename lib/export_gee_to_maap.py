...
def download_to_maap(fishnet, outdir, collection_items_df, tile_num):
    ...
    for index, row in fishnet_4326.iterrows():
        try:
            ...
            #print(f'Index: {index}, {minx}, {miny}, {maxx}, {maxy}')
...
def main():
    ...
    parser.add_argument("--subtile_index", type=int, required=True, help="The subtile index number provided by a fishnet over the GEE asset tile.")
    ...
    parser.add_argument("-a", "--asset_path", type=str, required=True, default="projects/foreststructure/Circumboreal/S1_Composites_albers", help="The GEE path to the asset collection")
    ...
    fishnet_df = create_fishnet(asset_df, args.tile_num, args.dims).to_crs(4326)
    
    # # Want to replace this with a download by subtile
    # fails = download_to_maap(fishnet_df, args.out_dir, asset_df, args.tile_num)
    
    # Do this...
    fails = do_gee_download_by_subtile(args.subtile_index, args.tile_num, fishnet_df, asset_df, args.out_dir)
